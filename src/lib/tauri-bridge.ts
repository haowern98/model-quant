import type { ModelInfo, TensorInfo, RecipeState, BenchmarkResult, AssignPattern, QuantType, RecipeSummary } from '../types';
...
export function setMockInvoke(fn: (cmd: string, args?: Record<string, unknown>) => Promise<unknown>): void {
  invokeFn = fn;
}
...
export async function assignQuant(tensorNames: string[], quantType: QuantType): Promise<RecipeState> {
...
export async function assignByPattern(pattern: AssignPattern, quantType: QuantType): Promise<RecipeState> {
  return invoke<RecipeState>('assign_by_pattern', { pattern, quantType });
}

export async function testRecipe(recipe: RecipeState, promptTokens: number): Promise<BenchmarkResult> {
  return invoke<BenchmarkResult>('test_recipe', { recipe, promptTokens });
}

export async function saveRecipe(path: string, recipe: RecipeState): Promise<void> {
  return invoke<void>('save_recipe', { path, recipe });
}

export async function exportGguf(path: string, recipe: RecipeState): Promise<void> {
