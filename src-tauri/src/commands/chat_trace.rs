...
fn save_trace_artifact_in(directory: &Path, artifact: &ChatTraceArtifact) -> Result<PathBuf, String> {
    ...
    let mut bytes = Vec::with_capacity(TRACE_MAGIC.len() + 4 + header.len() + artifact.payload.len());
    bytes.extend_from_slice(TRACE_MAGIC);
    bytes.extend_from_slice(&header_len.to_le_bytes());
    bytes.extend_from_slice(&header);
    bytes.extend_from_slice(&artifact.payload);

    let path = trace_artifact_path(directory, &artifact.conversation_id, &artifact.assistant_message_id)?;
...
fn load_trace_artifact_from(
    ...
    let header_len = u32::from_le_bytes(bytes[TRACE_MAGIC.len()..header_start].try_into().unwrap()) as usize;
    let header_end = header_start.checked_add(header_len).ok_or("Trace artifact is not valid.")?;
    let header = bytes.get(header_start..header_end).ok_or("Trace artifact is not valid.")?;
    let header: TraceHeader = serde_json::from_slice(header).map_err(|_| "Trace artifact is not valid.".to_string())?;
    if header.version != TRACE_VERSION || header.conversation_id != conversation_id || header.assistant_message_id != assistant_message_id {
        return Err("Trace artifact is not valid.".to_string());
    }
...
fn trace_artifact_path(directory: &Path, conversation_id: &str, assistant_message_id: &str) -> Result<PathBuf, String> {
    if !valid_id(conversation_id) || !valid_id(assistant_message_id) {
        return Err("Trace artifact id is invalid.".to_string());
    }
    Ok(directory.join(conversation_id).join(format!("{assistant_message_id}.trace")))
}
...
#[cfg(test)]
mod tests {
    use super::{load_trace_artifact_from, save_trace_artifact_in, trace_directory_from_local_app_data, ChatTraceArtifact};
    use std::fs;
    ...
    fn stores_trace_artifacts_in_the_mi_local_app_data_directory() {
        assert_eq!(
            trace_directory_from_local_app_data(Some(OsString::from(r"C:\\Users\\tester\\AppData\\Local"))),
    ...
    fn round_trips_a_trace_artifact() {
        let directory = std::env::temp_dir().join(format!("model-surgery-trace-test-{}", std::process::id()));
        ...
        assert_eq!(load_trace_artifact_from(&directory, "conversation-1", "assistant-1").unwrap(), artifact);
